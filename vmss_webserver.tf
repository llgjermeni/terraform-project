...
# Network interface for the webserver vmss
resource "azurerm_network_interface" "vmss-webserver" {
  name                = "vmss-nic"
  ...
  ip_configuration {
    ...
    public_ip_prefix_id           = azurerm_public_ip_prefix.vmss-server.id
  }
}

# Public IP for the webserver vmss
resource "azurerm_public_ip_prefix" "vmsss-server" {
  name                = "vmss-webserver-pip"
  location            = var.location
  resource_group_name = azurerm_resource_group.main.name
}
...
resource "azurerm_linux_virtual_machine_scale_set" "vmsss-api" {
  ...
  network_interface_ids = [
    azurerm_network_interface.vmss-webserver.id,
  ]
}
